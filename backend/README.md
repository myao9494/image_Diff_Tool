...
`/api/git/images` is the compatibility endpoint for changed images. `/api/git/files` lists changed images and configured text extensions, including added, deleted, and untracked files. `/api/git/item` returns one-sided image previews or structured side-by-side text rows. Text files are limited to 5 MB and 30,000 lines per side. Markdown files containing the `excalidraw-plugin: parsed` marker are also reported with `embedded_excalidraw: true`; `/api/git/diff` accepts `subresource: "excalidraw"` to compare the embedded drawing while the normal `/api/git/item` response remains the Markdown text diff. `/api/git/diff` compares the working-tree image to the HEAD-side image and accepts `head_path` for staged renames/copies.

`/api/git/revert-line` restores one displayed text-diff row in the working-tree file to its HEAD content. It strictly verifies the row kind, content, and old/new indexes against the current diff and returns `409` rather than editing a different line when the preview is stale.
